      ...
      if (Sauce.usersLiked.includes(req.body.userId)) {
        if (req.body.like === 0) {
          sauce.updateOne({ _id: Sauce._id }, { $pull: { usersLiked: req.body.userId }, $inc: { likes: -1 } })
            ...
            .catch(error => res.status(400).json({ error1: error }));
        } else if (req.body.like === -1) {
          sauce.updateOne({ _id: Sauce._id }, { $push: { usersDisliked: req.body.userId }, $pull: { usersLiked: req.body.userId }, $inc: { dislikes: 1, likes: -1 } })
        ...
        if (req.body.like === 1) {
          sauce.updateOne({ _id: Sauce._id }, { $push: { usersLiked: req.body.userId }, $pull: { usersDisliked: req.body.userId }, $inc: { likes: 1, dislikes: -1 } })
          ...
          sauce.updateOne({ _id: Sauce._id }, { $pull: { usersDisliked: req.body.userId }, $inc: { dislikes: -1 } })
            .then(() => res.status(200).json({ message: "neutre" }))
            .catch(error => res.status(400).json({ error4: error }));
        } else {
        ...
        if (req.body.like === 1) {
          sauce.updateOne({ _id: Sauce._id }, { $push: { usersLiked: req.body.userId }, $inc: { likes: 1 } })
            .then(() => res.status(200).json({ message: "like" }))
            .catch(error => res.status(400).json({ error5: error }));
        } else if (req.body.like === -1) {
          sauce.updateOne({ _id: Sauce._id }, { $push: { usersDisliked: req.body.userId }, $inc: { dislikes: 1 } })
            .then(() => res.status(200).json({ message: "dislike" }))
            .catch(error => res.status(400).json({ error6: error }));
        } else {
          res.status(200).json({ message: "neutre" })
        }
      }
    })
    .catch(error => res.status(400).json({ error: "c" }));
